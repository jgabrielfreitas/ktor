...
internal actual class SharedSet<T> {
    ...
    private var content = SharedVector<SharedVector<T>>()
    ...
    private fun initBuckets(count: Int) {
        ...
        repeat(count) {
            newContent.push(SharedVector<T>())
        }

        content = newContent
    }

    private fun findBucket(element: T): SharedVector<T> = content[element.hashCode() % content.size]
}
